...
public class PlaylistResource {
	...
	public Collection<Playlist> getAll(@QueryParam("isEmpty") 
	Boolean empty, @QueryParam("name") String name, @QueryParam("order") String order)
	{
		List<Playlist> returnedPlaylist = new ArrayList<>();
		for(Playlist playlist : repository.getAllPlaylists()) {
			if (name == null || playlist.getName().equals(name)) {
				if(empty == null || ((empty && playlist.getSongs()==null 
						&& playlist.getSongs().size() == 0 ) || 
						(!empty && playlist.getSongs()!=null && playlist.getSongs().size() != 0))){
					returnedPlaylist.add(playlist);
				}
			}
		}
